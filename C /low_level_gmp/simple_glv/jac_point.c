...
void point_from_GLVScalar(JacPoint *rop, GLVScalar *k, AffPoint *pp){
	...
	mpn_sub_n (neg_pp_Y[2], p_limbs, pp[2].Y_limbs, NB_LIMBS); // rop <- -pp[2].Y (p)
	mpn_sub_n (neg_pp_Y[3], p_limbs, pp[3].Y_limbs, NB_LIMBS); // rop <- -pp[3].Y (p)
	
	if(u < 0){
		mpn_copyi (rop->X_limbs, pp[-u-1].X_limbs, NB_LIMBS);
		mpn_copyi (rop->Y_limbs, neg_pp_Y[-u-1], NB_LIMBS);
	}
	else{
		mpn_copyi (rop->X_limbs, pp[u-1].X_limbs, NB_LIMBS);
		mpn_copyi (rop->Y_limbs, pp[u-1].Y_limbs, NB_LIMBS);
	}
	...
	while (j >= 0){
		...
		u = k->tk1[j] + 3 * k->tk2[j];
		if(u < 0){
			add_jac_aff(rop, pp[-u-1].X_limbs, neg_pp_Y[-u-1]);
		}
		else if(u > 0){
			add_jac_aff(rop, pp[u-1].X_limbs, pp[u-1].Y_limbs);
		}	
		j = j - 1;
	}
}
