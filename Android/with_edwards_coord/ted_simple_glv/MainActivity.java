...
//        beta is an element of order 4
    ...
    public void go(View v) {
        ...
        PP[1] = new ExtAffPoint(p);
        PP[3] = new ExtAffPoint(p);
        ...
        Debug.startMethodTracing("simple_glv_ted_", 1900*1024*1024);

        for (i = 0; i < nbiter; i++) {

            PP[2] = PP[0].endo(beta); /* ExtAffPoint PHI_P */
            PP[3].X = PP[2].X;
            PP[3].Y = PP[2].Y;
            PP[3].ADD(PP[0], curve_a, curve_d); /* ExtAffPoint P+PHI_P */
            PP[1].X = PP[0].X.negate();
            PP[1].Y = PP[0].Y;
            PP[1].ADD(PP[2], curve_a, curve_d); /* ExtAffPoint -P+PHI_P */
