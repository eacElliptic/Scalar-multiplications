...
void point_from_GLVScalar(ExtProjPoint *rop, GLVScalar *k, ExtAffPoint *pp){
	...
	if (k->tk1[j] < 0){
		...
		mpz_neg (rop->X, pp[u].x);
		mpz_neg (rop->T, pp[u].t);
	}
	else{
		mpz_set_ui (rop->Z, 1);
		mpz_set (rop->Y, pp[u].y);
		mpz_set (rop->X, pp[u].x);
		mpz_set (rop->T, pp[u].t);
	}
	...
	while (j > 0){
		...
		if (k->tk1[j] < 0){
			...
			add_extProj_extAff_to_Proj(rop, tmpX, pp[u].y, tmpT);
		}
		else {
			add_extProj_extAff_to_Proj(rop, pp[u].x, pp[u].y, pp[u].t);
		}
		j = j - 1;
	}
	...
	if (k->tk1[j] < 0){
		...
		add_extProj_extAff_to_extProj(rop, tmpX, pp[u].y, tmpT);
	}
	else {
		add_extProj_extAff_to_extProj(rop, pp[u].x, pp[u].y, pp[u].t);
	}
	
	
	if(k->isEven == 0)
		add_extProj_extAff_to_extProj(rop, pp[0].x, pp[0].y, pp[0].t);
